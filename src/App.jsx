    ...
    <div className="dream-root">
      ...
      <div className="dream-content">
        ...
        <div style={{marginBottom:28,borderTop:"1px solid rgba(255,255,255,0.07)",paddingTop:24}}>
          <div style={{fontFamily:"'Space Mono',monospace",fontSize:10,color:"rgba(255,255,255,0.45)",letterSpacing:3,textTransform:"uppercase",marginBottom:16}}>TRAVELER BRIEF</div>
          <div style={{marginBottom:20}}>
            <div style={{fontFamily:"'Space Mono',monospace",fontSize:9,color:"rgba(255,255,255,0.35)",letterSpacing:2,textTransform:"uppercase",marginBottom:8}}>WHO'S GOING</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
              {[{id:"solo",label:"Solo"},{id:"couple",label:"Couple / 2 Friends"}].map(g=>(
                <button key={g.id} onClick={()=>setTravelerGroup(g.id)} style={{padding:"9px 18px",borderRadius:24,border:travelerGroup===g.id?"1.5px solid rgba(255,159,67,0.75)":"1px solid rgba(255,255,255,0.12)",background:travelerGroup===g.id?"rgba(255,159,67,0.10)":"transparent",color:travelerGroup===g.id?"#FF9F43":"rgba(255,255,255,0.40)",fontSize:11,fontFamily:"'Space Mono',monospace",fontWeight:travelerGroup===g.id?600:400,cursor:"pointer",transition:"all 0.2s",minHeight:40,userSelect:"none"}}>{g.label}</button>
              ))}
            </div>
          </div>
          <div style={{marginBottom:20}}>
            <div style={{fontFamily:"'Space Mono',monospace",fontSize:9,color:"rgba(255,255,255,0.35)",letterSpacing:2,textTransform:"uppercase",marginBottom:8}}>YOUR TRAVEL STYLE</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
              {["First Timer","Independent Explorer","Comfort & Quality","Adventure First","Luxury"].map(s=>(
                <button key={s} onClick={()=>setTravelStyle(v=>v===s?"":s)} style={{padding:"9px 18px",borderRadius:24,border:travelStyle===s?"1.5px solid rgba(255,159,67,0.75)":"1px solid rgba(255,255,255,0.12)",background:travelStyle===s?"rgba(255,159,67,0.10)":"transparent",color:travelStyle===s?"#FF9F43":"rgba(255,255,255,0.40)",fontSize:10,fontFamily:"'Space Mono',monospace",fontWeight:travelStyle===s?600:400,cursor:"pointer",transition:"all 0.2s",minHeight:40,userSelect:"none"}}>{s}</button>
              ))}
            </div>
          </div>
          <div style={{marginBottom:0}}>
            <div style={{fontFamily:"'Space Mono',monospace",fontSize:9,color:"rgba(255,255,255,0.35)",letterSpacing:2,textTransform:"uppercase",marginBottom:8}}>INTERESTS <span style={{fontFamily:"'Fraunces',serif",fontStyle:"italic",fontWeight:300,color:"rgba(255,255,255,0.30)",fontSize:9,textTransform:"none"}}>· optional</span></div>
            <div style={{display:"flex",flexWrap:"wrap",gap:7}}>
              {[{id:"diving",icon:"🤿",label:"Diving"},{id:"culture",icon:"🌍",label:"Culture"},{id:"vlog",icon:"🎥",label:"Vlog"},{id:"food",icon:"🍜",label:"Food & Wine"},{id:"adventure",icon:"🥾",label:"Adventure"},{id:"golf",icon:"⛳",label:"Golf"},{id:"wellness",icon:"🧘",label:"Wellness"},{id:"remote",icon:"💻",label:"Remote Work"},{id:"safari",icon:"🦁",label:"Safari"}].map(c=>{const on=interests.includes(c.id);return(
                <button key={c.id} onClick={()=>setInterests(p=>on?p.filter(x=>x!==c.id):[...p,c.id])} style={{padding:"6px 12px",borderRadius:20,border:on?"1.5px solid rgba(255,159,67,0.65)":"1px solid rgba(255,255,255,0.10)",background:on?"rgba(255,159,67,0.08)":"transparent",color:on?"#FF9F43":"rgba(255,255,255,0.35)",fontSize:10,fontFamily:"'Space Mono',monospace",fontWeight:on?600:400,cursor:"pointer",transition:"all 0.2s",minHeight:40,userSelect:"none"}}>{c.icon} {c.label}</button>
              );})}
              <button onClick={()=>setSpecialtyOpen(o=>!o)} style={{padding:"6px 12px",borderRadius:20,border:specialtyInterests.length>0?"1.5px solid rgba(255,159,67,0.65)":specialtyOpen?"1px solid rgba(255,159,67,0.4)":"1px solid rgba(255,255,255,0.10)",background:specialtyInterests.length>0?"rgba(255,159,67,0.08)":"transparent",color:specialtyInterests.length>0?"#FF9F43":specialtyOpen?"rgba(255,159,67,0.6)":"rgba(255,255,255,0.35)",fontSize:10,fontFamily:"'Space Mono',monospace",fontWeight:specialtyInterests.length>0?600:400,cursor:"pointer",transition:"all 0.2s",minHeight:40,userSelect:"none"}}>✦ Specialty{specialtyInterests.length>0?` (${specialtyInterests.length})`:"..."}</button>
            </div>
            <div style={{maxHeight:specialtyOpen?300:0,overflow:"hidden",transition:"max-height 0.28s ease-out"}}>
              <div style={{paddingTop:12,marginTop:4,borderTop:"1px solid rgba(255,255,255,0.06)"}}>
                <div style={{fontFamily:"'Space Mono',monospace",fontSize:9,color:"rgba(255,255,255,0.35)",letterSpacing:2,marginBottom:8}}>SPECIALTY INTERESTS <span style={{fontFamily:"'Fraunces',serif",fontStyle:"italic",fontWeight:300,color:"rgba(255,255,255,0.30)",fontSize:9}}>· optional</span></div>
                <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                  {[{id:"fishing",icon:"🎣",label:"Fishing"},{id:"climbing",icon:"🧗",label:"Climbing"},{id:"skiing",icon:"🎿",label:"Skiing/Snow"},{id:"nightlife",icon:"🎉",label:"Nightlife"},{id:"music",icon:"🎵",label:"Music/Festivals"},{id:"wine",icon:"🍷",label:"Wine Tourism"},{id:"eco",icon:"🌿",label:"Eco Travel"},{id:"photography",icon:"🎨",label:"Photography"},{id:"camping",icon:"🏕️",label:"Camping"},{id:"yoga",icon:"🤸",label:"Yoga Retreat"},{id:"watersports",icon:"🏄",label:"Water Sports"}].map(c=>{const on=specialtyInterests.includes(c.id);return(
                    <button key={c.id} onClick={()=>setSpecialtyInterests(p=>on?p.filter(x=>x!==c.id):[...p,c.id])} style={{padding:"5px 9px",borderRadius:20,border:on?"1.5px solid rgba(255,159,67,0.65)":"1px solid rgba(255,255,255,0.10)",background:on?"rgba(255,159,67,0.08)":"transparent",color:on?"#FF9F43":"rgba(255,255,255,0.35)",fontSize:10,fontFamily:"'Space Mono',monospace",fontWeight:on?600:400,cursor:"pointer",transition:"all 0.2s",minHeight:40,userSelect:"none"}}>{c.icon} {c.label}</button>
                  );})}
                </div>
              </div>
            </div>
