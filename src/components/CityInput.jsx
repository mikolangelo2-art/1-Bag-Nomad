...
function CityInput({value,onChange,className,style:inputStyle,placeholder,accent,onFocus,onBlur}){
  ...
  return(<div style={{position:'relative'}}><input className={className} style={inputStyle} value={value} onChange={e=>handleChange(e.target.value)} onKeyDown={handleKey} onFocus={e=>{setFocused(true);onFocus?.(e);}} onBlur={e=>{setTimeout(()=>setFocused(false),180);onBlur?.(e);}} placeholder={placeholder} autoComplete="new-password" autoCorrect="off" autoCapitalize="off" spellCheck="false"/>
    {focused&&suggestions.length>0&&<div style={{position:'absolute',top:'100%',left:0,right:0,zIndex:50,background:'rgba(21,15,10,0.97)',border:`1.5px solid ${borderAccent}`,borderRadius:10,marginTop:4,overflow:'hidden',boxShadow:accent==="cyan"?'0 8px 32px rgba(0,0,0,0.6),0 0 20px rgba(0,229,255,0.12)':'0 8px 32px rgba(0,0,0,0.6),0 0 20px rgba(255,159,67,0.08)'}}>
