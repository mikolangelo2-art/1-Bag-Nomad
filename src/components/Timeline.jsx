  ...
  return (
    <div style={{padding:"20px 0 40px",position:"relative"}}>
      ...
      <div style={{textAlign:"center",marginBottom:28}}>
        <div style={{fontFamily:"'Inter',system-ui,-apple-system,sans-serif",fontSize:10,color:"rgba(255,217,61,0.6)",letterSpacing:3,marginBottom:4}}>TOTAL EXPEDITION BUDGET</div>
        <div style={{fontFamily:"'Fraunces',serif",fontSize:24,color:"#FFD93D",fontWeight:700}}>{fmt(totalBudget)}</div>
      ...
      <div style={{position:"relative",paddingLeft:28}}>
        ...
        <div style={{position:"relative",marginBottom:24,animation:"fadeUp 0.4s ease both"}}>
          ...
          <div style={{fontFamily:"'Inter',system-ui,-apple-system,sans-serif",fontSize:10,color:"rgba(255,217,61,0.6)",letterSpacing:3,marginBottom:4}}>DEPARTS</div>
          <div style={{fontFamily:"'Fraunces',serif",fontSize:22,color:"#E8DCC8",fontWeight:500}}>{startDate ? fD(startDate) : "Date TBD"}</div>
          {daysFromNow !== null && daysFromNow > 0 && <div style={{fontFamily:"'Inter',system-ui,-apple-system,sans-serif",fontSize:12,color:"#FFD93D",marginTop:4}}>{daysFromNow} DAYS FROM NOW</div>}
          {daysFromNow !== null && daysFromNow <= 0 && <div style={{fontFamily:"'Inter',system-ui,-apple-system,sans-serif",fontSize:12,color:"#69F0AE",marginTop:4}}>EXPEDITION UNDERWAY</div>}
        </div>
          ...
          <div key={phase.id || i}>
            ...
            <div style={{position:"relative",marginBottom:24,animation:"fadeUp 0.4s ease both",animationDelay:i * 0.1 + "s"}}>
              ...
              <div style={{display:"flex",alignItems:"baseline",gap:8,marginBottom:2}}>
                {phase.flag && <span style={{fontSize:16}}>{phase.flag}</span>}
                <div style={{fontFamily:"'Fraunces',serif",fontSize:18,color:"#E8DCC8",fontWeight:500}}>{phase.name || phase.destination}</div>
              </div>
              <div style={{fontFamily:"'Inter',system-ui,-apple-system,sans-serif",fontSize:12,color:"rgba(255,255,255,0.55)",marginBottom:3}}>
                {phase.arrival && phase.departure ? `${fD(phase.arrival)} – ${fD(phase.departure)}` : "Dates TBD"}
              </div>
              <div style={{fontFamily:"'Inter',system-ui,-apple-system,sans-serif",fontSize:12,color:"rgba(255,255,255,0.45)",display:"flex",gap:8}}>
                <span>{phase.nights}n</span>
                <span style={{color:"#FFD93D"}}>{fmt(phase.budget || phase.cost || 0)}</span>
              </div>
            </div>
          </div>
        ))}
        ...
        <div style={{position:"relative",animation:"fadeUp 0.4s ease both",animationDelay:phases.length * 0.1 + "s"}}>
          <div style={{position:"absolute",left:-22,top:4,width:8,height:8,borderRadius:"50%",background:"#FFD93D",boxShadow:"0 0 8px rgba(255,217,61,0.5)"}}/>
          <div style={{fontFamily:"'Inter',system-ui,-apple-system,sans-serif",fontSize:10,color:"rgba(255,217,61,0.6)",letterSpacing:3,marginBottom:4}}>HOMECOMING</div>
          {returnDate
            ? <div style={{fontFamily:"'Fraunces',serif",fontSize:16,fontStyle:"italic",color:"#FFD93D"}}>{fD(returnDate)}</div>
            : <div style={{fontFamily:"'Fraunces',serif",fontSize:16,fontStyle:"italic",color:"rgba(255,217,61,0.5)"}}>Open-ended expedition</div>
          }
        </div>
      </div>
    </div>
