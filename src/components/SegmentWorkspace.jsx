...
function CollapsibleSuggestion({summaryCard=false,onSwitchToSuggestion,confirmMessage,children}){
  ...
  return(
    <div style={{marginTop:10}}>
      <button type="button" onClick={()=>setOpen(o=>!o)} style={{background:'none',border:'none',color:'rgba(201,160,76,0.65)',fontSize:13,cursor:'pointer',padding:0,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",letterSpacing:'0.1em',textAlign:'left',width:'100%',lineHeight:1.45}}>{open?'⌄ Hide suggestion':'› View suggestion'}</button>
...
function SegmentWorkspace({segment,phaseId,phaseName:phaseLabelName,phaseFlag,intelSnippet,onBack,onBackToExpedition,suggestion:suggestionProp,suggestionsLoading,homeCity="",prevCity="",allPhases=[]}) {
  ...
  const planCommitLabelStyle={fontSize:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",color:'rgba(201,160,76,0.65)',letterSpacing:2};
  const planCommitAddedLineStyle={fontSize:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",color:'rgba(201,160,76,0.92)',letterSpacing:0.35,lineHeight:1.45};
  const savedCheckStyle={color:'#69F0AE',fontSize:14,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontWeight:700};
  const savedSubStyle={color:'rgba(255,255,255,0.5)',fontSize:12,marginTop:4,lineHeight:1.5,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif"};
  ...
  const addedPlanLineStyle={fontSize:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",color:'rgba(105,240,174,0.88)',letterSpacing:0.35,lineHeight:1.45};
  const returnToLogFooterStyle={fontSize:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",color:'rgba(255,255,255,0.36)',letterSpacing:'0.14em',lineHeight:1.58,marginTop:8};
  ...
  const tripFieldLabel={fontSize:isMobile?12:14,color:"rgba(94,139,138,0.75)",letterSpacing:1.5,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontWeight:500,opacity:0.92};
  const cityInStyle={background:"rgba(0,0,0,0.55)",border:"1px solid rgba(255,255,255,0.22)",borderRadius:6,color:"#FFF",fontSize:isMobile?12:15,padding:isMobile?"4px 7px":"5px 8px",fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",outline:"none",width:"100%",maxWidth:"100%",boxSizing:"border-box",lineHeight:1.6};
  const legChipStyle={padding:"5px 10px",borderRadius:6,border:"1px solid rgba(94,139,138,0.35)",background:"rgba(94,139,138,0.06)",color:"rgba(94,139,138,0.88)",fontSize:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontWeight:600,cursor:"pointer",whiteSpace:"nowrap",minHeight:32,lineHeight:1.35};
  ...
  const transportDateTimeGrid=<>
    <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 1fr',gap:10,marginTop:10}}>
      <div style={{display:'flex',gap:8,alignItems:'flex-end',flexWrap:'wrap'}}>
        <div style={{flex:'1 1 160px',minWidth:0}}><SDF type="date" label="DEPART DATE" value={det.transport.departDate??""} onChange={v=>uT("departDate",v)} accent="#C9A04C"/></div>
        <div style={{flex:'0 1 104px',minWidth:84}}><SDF label="DEPART TIME" value={det.transport.departTime??""} onChange={v=>uT("departTime",v)} placeholder="HH:MM" accent="#C9A04C"/></div>
      </div>
      <div style={{display:'flex',gap:8,alignItems:'flex-end',flexWrap:'wrap'}}>
        <div style={{flex:'1 1 160px',minWidth:0}}><SDF type="date" label="ARRIVE DATE" value={det.transport.arriveDate??""} onChange={v=>uT("arriveDate",v)} accent="#C9A04C"/></div>
        <div style={{flex:'0 1 104px',minWidth:84}}><SDF label="ARRIVE TIME" value={det.transport.arriveTime??""} onChange={v=>uT("arriveTime",v)} placeholder="HH:MM" accent="#C9A04C"/></div>
      </div>
    </div>
  </>;
  ...
  return(
    <div style={{position:'fixed',top:0,left:0,right:0,bottom:0,zIndex:300,background:BG_PAGE,overflowY:'auto',animation:'slideInRight 0.45s cubic-bezier(0.25,0.46,0.45,0.94)'}}>
        ...
        {isMobile?(
          <>
            ...
            <div style={{flex:1,minWidth:0}}>
              ...
              <div style={{fontSize:13,color:'rgba(255,255,255,0.72)',fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",marginTop:2}}>{`${segment.nights}n`} · {segment.type} · {fmt(segment.budget)}</div>
            </div>
          </>
        ):(
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',width:'100%',gap:16,flexWrap:'wrap'}}>
            <div style={{display:'flex',alignItems:'center',gap:8,flex:'1 1 220px',minWidth:0,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif"}}>
            ...
            <div style={{display:'flex',alignItems:'stretch',gap:2,border:'1px solid rgba(255,255,255,0.10)',borderRadius:12,overflow:'hidden',background:'rgba(10,7,5,0.45)',flexShrink:0,boxShadow:'inset 0 1px 0 rgba(255,255,255,0.04)'}}>
              <div style={{padding:'8px 14px',textAlign:'center',minWidth:76}}>
                <div style={{fontSize:10,letterSpacing:1.5,color:'rgba(255,255,255,0.45)',fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontWeight:600,marginBottom:3}}>NIGHTS</div>
                <div style={{fontSize:15,fontWeight:600,color:'#F8F5F0',fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",lineHeight:1.15}}>{segment.nights}</div>
              </div>
              <div style={{borderLeft:'1px solid rgba(255,255,255,0.08)',padding:'8px 14px',textAlign:'center',minWidth:88,maxWidth:150}}>
                <div style={{fontSize:10,letterSpacing:1.5,color:'rgba(255,255,255,0.45)',fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontWeight:600,marginBottom:3}}>STYLE</div>
                <div style={{fontSize:15,fontWeight:600,color:'#F8F5F0',fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",lineHeight:1.15,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}} title={segment.type}>{segment.type}</div>
              </div>
              <div style={{borderLeft:'1px solid rgba(255,255,255,0.08)',padding:'8px 14px',textAlign:'center',minWidth:88}}>
                <div style={{fontSize:10,letterSpacing:1.5,color:'rgba(255,255,255,0.45)',fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontWeight:600,marginBottom:3}}>BUDGET</div>
                <div style={{fontSize:15,fontWeight:600,color:'#C9A04C',fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",lineHeight:1.15}}>{fmt(segment.budget)}</div>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="sw-pill-tab-row" style={tabBarStyle}>
        ...
        {saveFlash&&<div style={{position:'absolute',right:8,top:8,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontSize:13,color:'#69F0AE',opacity:0.80,letterSpacing:1,pointerEvents:'none'}}>✓ saved</div>}
      </div>
      {/* Tab content — visited tabs stay mounted; visibility toggles (Session 51H) */}
      <div style={{border:segMobileLoose?'none':'1.5px solid rgba(255,255,255,0.10)',borderRadius:segWorkspaceHdrR,background:'rgba(10,7,5,0.85)',padding:segMobileLoose?'10px 10px':'16px 14px',margin:segMobileLoose?'6px 0':'12px 0',minHeight:300,textAlign:'left',animation:'tabFadeIn 400ms cubic-bezier(0.25,0.46,0.45,0.94)',overflow:'hidden'}}>
        {(tab==="transport"||tab==="stay"||tab==="activities"||tab==="food")&&!!segmentCardDateHeader&&<div style={{textAlign:'center',margin:segMobileLoose?'0 0 10px':'0 0 14px',padding:segMobileLoose?'0 0 10px':'0 8px 12px',borderBottom:'1px solid rgba(255,255,255,0.08)'}}>
          <div style={{fontSize:isMobile?13:14,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",color:'rgba(255,255,255,0.52)',letterSpacing:0.35,lineHeight:1.52}}>{segmentCardDateHeader}</div>
        </div>}
        {/* TRANSPORT */}
        {visitedTabs.has("transport")&&<div style={{display:tab==="transport"?"block":"none",padding:0}}>
          ...
          {suggestionsLoading&&!suggestion&&<div style={{padding:'12px 16px',marginBottom:16,border:'none',borderRadius:12,background:'transparent',display:'flex',alignItems:'center',gap:10}}>
            <div style={{width:8,height:8,borderRadius:'50%',background:'rgba(201,160,76,0.6)',animation:'pulse 1.5s ease-in-out infinite'}}/>
            <span style={{fontSize:13,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",color:'rgba(255,255,255,0.60)',letterSpacing:1,lineHeight:1.45}}>CO-ARCHITECT IS PREPARING YOUR SUGGESTIONS...</span>
          </div>}
          {suggestion?.transport&&!isDism('transport')&&(!hideTransportSuggestion||transportRouteInPlace)&&<div ref={transportSuggestionRef} style={transportRouteInPlace?{border:planCommitCardBorder,borderRadius:planCommitCardRadius,background:planCommitCardBg,padding:planCommitCardPad,marginBottom:14,display:'flex',flexDirection:'column'}:{...suggestionCardStyle,background:'transparent',boxShadow:'none',border:'none'}}>
            ...
            <div style={{display:'flex',alignItems:'center',marginBottom:10}}>
              ...
              <button type="button" onClick={()=>{setBookDropdown(bookDropdown==='transport'?null:'transport');}} style={{background:'none',border:'1px solid rgba(94,139,138,0.25)',borderRadius:6,color:'rgba(94,139,138,0.60)',fontSize:11,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontWeight:600,letterSpacing:1,padding:'4px 10px',cursor:'pointer',minHeight:28,marginRight:6}}>🔗</button>
              <button type="button" onClick={()=>setEditingTransport(e=>!e)} style={{background:'none',border:'1px solid rgba(201,160,76,0.30)',borderRadius:6,color:'rgba(201,160,76,0.70)',fontSize:11,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontWeight:600,letterSpacing:1,padding:'4px 10px',cursor:'pointer',minHeight:28,marginRight:6}}>{editingTransport?'DONE':'EDIT'}</button>
              <button type="button" onClick={clearTransport} style={{background:'none',border:'1px solid rgba(255,255,255,0.15)',borderRadius:6,color:'rgba(255,255,255,0.35)',fontSize:11,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",padding:'4px 10px',cursor:'pointer',minHeight:28}}>✕</button>
            </div>
            ...
            {!!travelSummaryDateLine&&<div style={{fontSize:14,color:'rgba(255,255,255,0.78)',fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",marginBottom:4,lineHeight:1.5}}>{travelSummaryDateLine}</div>}
            {transportCostModeParts.length>0&&<div style={{fontSize:14,color:'#C9A04C',fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",marginBottom:8,lineHeight:1.45}}>{transportCostModeParts.join(' · ')}</div>}
            ...
            {det.transport.notes&&!editingTransport&&<div style={{fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontSize:13,color:'rgba(255,255,255,0.60)',marginTop:8,lineHeight:1.52,whiteSpace:'pre-line'}}>{det.transport.notes.length>120?det.transport.notes.slice(0,120)+'...':det.transport.notes}</div>}
            {det.transport.link&&<a href={det.transport.link} target="_blank" rel="noopener noreferrer" style={{fontSize:12,color:'#5E8B8A',textDecoration:'none',display:'inline-block',marginTop:4}}>{det.transport.link.replace(/^https?:\/\//,"").slice(0,40)}</a>}
            {!editingTransport&&suggestion?.transport&&<CollapsibleSuggestion summaryCard confirmMessage="Replace your saved transport with this Co-Architect suggestion?" onSwitchToSuggestion={()=>acceptTransport(suggestion.transport)}>
              ...
              <div style={{fontSize:14,fontWeight:600,color:'#c9a04c',marginBottom:6,lineHeight:1.48,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif"}}>Est. {suggestion.transport.estimatedCost} · {suggestion.transport.duration||""}</div>
            ...
            <div style={{fontSize:10,color:'rgba(201,160,76,0.9)',letterSpacing:2,marginBottom:8,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif"}}>SUGGESTED ROUTE</div>
            <div style={suggestionHeaderReadable}>✦ CO-ARCHITECT SUGGESTION</div>
            <div style={{fontSize:14,fontWeight:700,color:'#c9a04c',marginBottom:8,lineHeight:1.5,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif"}}>
            ...
            <div style={{fontSize:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",color:'rgba(255,255,255,0.42)',lineHeight:1.52,marginBottom:6}}>{transportSuggestionEstimateHint({prevCity,segmentName:segment.name})}</div>
          ...
          {transportLegLocked&&!transportRouteInPlace&&<div ref={transportCommittedRef} style={{border:planCommitCardBorder,borderRadius:planCommitCardRadius,background:planCommitCardBg,padding:planCommitCardPad,marginBottom:14,display:'flex',flexDirection:'column'}}>
            ...
            {!!travelSummaryDateLine&&<div style={{fontSize:14,color:'rgba(255,255,255,0.78)',fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",marginBottom:4,lineHeight:1.5}}>{travelSummaryDateLine}</div>}
            {transportCostModeParts.length>0&&<div style={{fontSize:14,color:'#C9A04C',fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",marginBottom:4,lineHeight:1.45}}>{transportCostModeParts.join(' · ')}</div>}
            <div style={savedCheckStyle}>✓ Route added to your plan.</div>
            <div style={{...savedSubStyle,display:'block'}}>Booking links and confirmation details can be added anytime.</div>
            {det.transport.notes&&!editingTransport&&<div style={{fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontSize:13,color:'rgba(255,255,255,0.60)',marginTop:6,lineHeight:1.52,whiteSpace:'pre-line'}}>{det.transport.notes.length>120?det.transport.notes.slice(0,120)+'...':det.transport.notes}</div>}
            {det.transport.link&&<a href={det.transport.link} target="_blank" rel="noopener noreferrer" style={{fontSize:12,color:'#5E8B8A',textDecoration:'none',display:'inline-block',marginTop:4}}>{det.transport.link.replace(/^https?:\/\//,"").slice(0,40)}</a>}
            </div>
            {!editingTransport&&suggestion?.transport&&<CollapsibleSuggestion summaryCard confirmMessage="Replace your saved transport with this Co-Architect suggestion?" onSwitchToSuggestion={()=>acceptTransport(suggestion.transport)}>
              <div style={suggestionHeaderReadable}>✦ CO-ARCHITECT SUGGESTION</div>
          ...
          {bookDropdown==='transport'&&<div style={{position:'relative',zIndex:100,marginBottom:10}}><div style={{background:'rgba(10,7,5,0.95)',border:'1px solid rgba(255,255,255,0.12)',borderRadius:12,padding:8,boxShadow:'0 8px 32px rgba(0,0,0,0.6)'}}>
          ...
          {transportLegLocked&&editingTransport&&<div style={{border:'none',borderRadius:12,background:'transparent',padding:16,marginBottom:14,animation:'slideOpen 0.40s cubic-bezier(0.25,0.46,0.45,0.94)'}}>
            <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 1fr',gap:10}}>
              <SDF label="MODE" value={det.transport.mode} onChange={v=>uT("mode",v)} placeholder="Flight / Ferry / Car..." accent="#C9A04C"/>
              <SDF label="COST ($)" type="number" value={det.transport.cost} onChange={v=>uT("cost",v)} placeholder="0" accent="#C9A04C"/>
            </div>
            {transportFromToGrid(false)}
            {transportDateTimeGrid}
            <div style={{marginTop:10}}><SDF label="BOOKING LINK" value={det.transport.link||""} onChange={v=>uT("link",v)} placeholder="https://..." accent="#C9A04C"/></div>
            <div style={{marginTop:8}}><SDF label="NOTES" value={det.transport.notes} onChange={v=>uT("notes",v)} placeholder="e.g. 1) Flight AA… 2) Shuttle to pier 3) Utila ferry — refs & costs per leg" accent="#C9A04C" multiline/></div>
            <button type="button" onClick={()=>setEditingTransport(false)} style={{marginTop:10,width:'100%',padding:'10px',borderRadius:8,border:'none',background:'rgba(94,139,138,0.12)',color:'#5E8B8A',fontSize:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontWeight:700,letterSpacing:1,cursor:'pointer',minHeight:40}}>SAVE CHANGES</button>
          </div>}
          {needsTravelManualToggle&&<button type="button" onClick={()=>setTravelManualOpen(o=>!o)} style={{width:'100%',textAlign:'left',background:'transparent',border:'1px solid rgba(94,139,138,0.35)',borderRadius:10,color:'rgba(94,139,138,0.85)',fontSize:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontWeight:700,letterSpacing:1,padding:'10px 14px',cursor:'pointer',marginBottom:10,minHeight:44}}>{travelManualOpen||planningOwn?'⌄ Hide manual route':'+ Plan your own route'}</button>}
          {showTravelManualInner&&<div>
            ...
            {(transportEstLoading||transportEst)&&<div style={{padding:'8px 12px',marginTop:8,borderRadius:8,background:'transparent',border:'none',display:'flex',flexDirection:'column',alignItems:'flex-start',gap:6}}>
              {transportEstLoading?<span style={{fontSize:13,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",color:'rgba(201,160,76,0.60)',letterSpacing:1,lineHeight:1.45}}>✦ Estimating...</span>
              :transportEst&&<span style={{fontSize:13,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",color:'rgba(201,160,76,0.65)',letterSpacing:0.5,lineHeight:1.52}}>
                ✦ Est. {transportEst.estimate}{transportEst.note?` — ${transportEst.note}`:""}{" "}
                <button type="button" onClick={()=>{const d=parseTransportEstimateToCostDigits(transportEst.estimate);if(d)uT("cost",d);}} style={{background:'none',border:'none',padding:0,color:'#C9A04C',textDecoration:'underline',cursor:'pointer',fontSize:'inherit',fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontWeight:600}}>use</button>
            ...
            <div style={{fontSize:12,color:'rgba(255,255,255,0.38)',lineHeight:1.58,marginTop:6,marginBottom:2,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif"}}>
              More than one hop (flight + bus + ferry)? Use <span style={{color:'rgba(94,139,138,0.72)'}}>NOTES</span> to list each leg, times, and refs; <span style={{color:'rgba(94,139,138,0.72)'}}>BOOKING LINK</span> for your main ticket or a trip folder. <span style={{color:'rgba(255,255,255,0.28)'}}>ACTIVITIES</span> is for things you do at this stop, not airport transfers.
            </div>
            {transportDateTimeGrid}
            <div style={{marginTop:10}}><SDF label="BOOKING LINK" value={det.transport.link||""} onChange={v=>uT("link",v)} placeholder="https://..." accent="#C9A04C"/></div>
            <div style={{marginTop:8}}><SDF label="NOTES" value={det.transport.notes} onChange={v=>uT("notes",v)} placeholder="e.g. 1) Flight AA… 2) Shuttle to pier 3) Utila ferry — refs & costs per leg" accent="#C9A04C" multiline/></div>
            {(()=>{const ready=!isRowEmpty(det.transport?.from)&&!isRowEmpty(det.transport?.to);return <button type="button" disabled={!ready} onClick={saveManualTransport} style={{marginTop:12,width:'100%',padding:'12px',borderRadius:8,border:'1px solid '+(ready?'rgba(94,139,138,0.45)':'rgba(255,255,255,0.10)'),background:ready?'rgba(94,139,138,0.12)':'rgba(255,255,255,0.03)',color:ready?'#5E8B8A':'rgba(255,255,255,0.30)',fontSize:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontWeight:700,letterSpacing:1.5,cursor:ready?'pointer':'not-allowed',minHeight:44}}>SAVE TRANSPORT</button>;})()}
            {suggestion?.transport&&isDism('transport')&&<CollapsibleSuggestion>
              <div style={suggestionHeaderReadable}>✦ CO-ARCHITECT SUGGESTION</div>
              <div style={{fontSize:14,fontWeight:700,color:'#c9a04c',marginBottom:8,lineHeight:1.5,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif"}}>This leg: {plannedLegFrom} → {plannedLegTo}<span style={{color:'rgba(255,245,220,0.45)',fontWeight:600}}> · </span>Est. {suggestion.transport.estimatedCost}</div>
              {suggestion.transport.route?<div style={{fontSize:14,color:'rgba(255,255,255,0.36)',lineHeight:1.58,marginBottom:6,whiteSpace:'pre-wrap'}}>{suggestion.transport.route}</div>:null}
              <div style={{fontSize:14,color:'rgba(255,255,255,0.75)',marginBottom:4,lineHeight:1.45}}>{suggestion.transport.duration}</div>
              {suggestion.transport.notes&&<div style={{fontSize:14,color:'rgba(255,255,255,0.70)',fontStyle:'italic',marginBottom:12,lineHeight:1.5}}>{suggestion.transport.notes}</div>}
              <div style={disclaimerReadable}>⚡ Estimates based on current market rates — actual prices vary when booked</div>
        ...
        {visitedTabs.has("stay")&&<div style={{display:tab==="stay"?"block":"none",padding:0}}>
          ...
          {showStayAccommodationCard&&stayInsightSavedIdx==null&&<div style={{border:planCommitCardBorder,borderRadius:planCommitCardRadius,background:planCommitCardBg,padding:planCommitCardPad,marginBottom:14,display:'flex',flexDirection:'column'}}>
            ...
            <div style={{display:'flex',alignItems:'center',marginBottom:8}}>
              ...
              <button type="button" title="Edit details" onClick={()=>setEditingStay(e=>!e)} style={{background:'none',border:'1px solid rgba(201,160,76,0.30)',borderRadius:6,color:'rgba(201,160,76,0.70)',fontSize:11,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontWeight:600,letterSpacing:1,padding:'4px 10px',cursor:'pointer',minHeight:28,marginRight:6}}>{editingStay?'DONE':'EDIT'}</button>
              <button type="button" title="Remove from plan" onClick={clearStayPlan} style={{background:'none',border:'1px solid rgba(255,255,255,0.15)',borderRadius:6,color:'rgba(255,255,255,0.35)',fontSize:11,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",padding:'4px 10px',cursor:'pointer',minHeight:28}}>✕</button>
            </div>
            ...
            {det.stay.notes&&!editingStay&&<div style={{fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontSize:13,fontStyle:'italic',color:'rgba(255,255,255,0.75)',marginBottom:8,marginTop:4,lineHeight:1.6}}>{(()=>{const n=sanitizeAiDisplayText(det.stay.notes);const first=(n.split(/\n/)[0]||"").trim();return first.length>200?first.slice(0,200)+'…':first;})()}</div>}
            {det.stay.checkin&&det.stay.checkout&&<div style={{fontSize:13,color:'#C9A04C',fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",display:'flex',gap:8,flexWrap:'wrap',alignItems:'center',marginBottom:det.stay.link?6:0}}><span style={{fontStyle:'italic',color:'rgba(201,160,76,0.60)',fontSize:12}}>{fD(det.stay.checkin)} – {fD(det.stay.checkout)}</span><span style={{color:'#c9a04c'}}>· {segment.nights} nights</span></div>}
            {det.stay.link&&<a href={det.stay.link} target="_blank" rel="noopener noreferrer" style={{fontSize:12,color:'#5E8B8A',textDecoration:'none',display:'inline-block',marginTop:4}}>{det.stay.link.replace(/^https?:\/\//,"").slice(0,40)}</a>}
          ...
          {bookDropdown==='stay'&&<div style={{position:'relative',zIndex:100,marginBottom:10}}><div style={{background:'rgba(10,7,5,0.95)',border:'1px solid rgba(255,255,255,0.12)',borderRadius:12,padding:8,boxShadow:'0 8px 32px rgba(0,0,0,0.6)'}}>
          ...
          {(!showStayAccommodationCard||stayInsightSavedIdx!=null)&&(
            ...
            <>
                  ...
                  <>
                    ...
                    {stayInsightSavedIdx!=null&&showStayBookingForm&&(
                      <div style={{border:'1px solid rgba(255,255,255,0.10)',borderRadius:12,background:'rgba(255,255,255,0.04)',padding:16,marginBottom:14,animation:'slideOpen 0.40s cubic-bezier(0.25,0.46,0.45,0.94)'}}>
                        ...
                        <button type="button" onClick={()=>setShowStayBookingForm(false)} style={{marginTop:10,width:'100%',padding:'10px',borderRadius:8,border:'none',background:'rgba(105,240,174,0.12)',color:'#69F0AE',fontSize:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontWeight:700,letterSpacing:1,cursor:'pointer',minHeight:40}}>SAVE CHANGES</button>
                      </div>
                    )}
              ...
              {stayInsightSavedIdx!=null&&<button type="button" onClick={()=>setStayManualOpen(o=>!o)} style={{width:'100%',textAlign:'left',background:'rgba(255,255,255,0.04)',border:'1px solid rgba(201,160,76,0.35)',borderRadius:10,color:'#c9a04c',fontSize:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontWeight:700,letterSpacing:1,padding:'10px 14px',cursor:'pointer',marginTop:8,marginBottom:4,minHeight:44}}>{stayManualOpen?'⌄ Hide manual booking':'+ Add another stay'}</button>}
            </>
            </SuggestionPhotoDedupProvider>
          )}
          {showStayAccommodationCard&&editingStay&&<div style={{border:'1px solid rgba(255,255,255,0.10)',borderRadius:12,background:'rgba(255,255,255,0.04)',padding:16,marginBottom:14,animation:'slideOpen 0.40s cubic-bezier(0.25,0.46,0.45,0.94)'}}>
            ...
            <div style={{marginTop:10}}><SDF label="BOOKING LINK" value={det.stay.link} onChange={v=>uS("link",v)} placeholder="https://..." accent="#69F0AE"/></div>
            <div style={{marginTop:8}}><SDF label="NOTES" value={det.stay.notes} onChange={v=>uS("notes",v)} placeholder="Room type, included meals, host contact..." accent="#69F0AE" multiline/></div>
            <button type="button" onClick={()=>setEditingStay(false)} style={{marginTop:10,width:'100%',padding:'10px',borderRadius:8,border:'none',background:'rgba(105,240,174,0.12)',color:'#69F0AE',fontSize:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontWeight:700,letterSpacing:1,cursor:'pointer',minHeight:40}}>SAVE CHANGES</button>
          </div>}
          {(stayManualOpen||planningOwnStay)&&(stayInsightSavedIdx!=null||!showStayAccommodationCard)&&<>
            ...
            {!showStayAccommodationCard&&(hasS||hasStaySecondaryField)&&<div style={{marginTop:12}}>
              <button type="button" onClick={()=>{if(typeof document!=="undefined")document.activeElement?.blur();setStayFocused(false);}} style={{padding:'8px 14px',borderRadius:6,border:'1px solid rgba(201,160,76,0.30)',background:'none',color:'rgba(201,160,76,0.70)',fontSize:12,cursor:'pointer',fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",letterSpacing:1,fontWeight:600,whiteSpace:'nowrap',height:34}}>SAVE</button>
            </div>}
          </div>}
          </>}
        </div>}
        ...
        {visitedTabs.has("activities")&&<div style={{display:tab==="activities"?"block":"none",padding:0}}>
          ...
          {suggestionsLoading&&!suggestion&&<div style={{padding:'12px 16px',marginBottom:16,border:'1px solid rgba(201,160,76,0.15)',borderRadius:12,background:'rgba(201,160,76,0.03)',display:'flex',alignItems:'center',gap:10}}>
            <div style={{width:8,height:8,borderRadius:'50%',background:'rgba(201,160,76,0.6)',animation:'pulse 1.5s ease-in-out infinite'}}/>
            <span style={{fontSize:13,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",color:'rgba(255,255,255,0.60)',letterSpacing:1,lineHeight:1.45}}>CO-ARCHITECT IS PREPARING YOUR SUGGESTIONS...</span>
          </div>}
          <SuggestionPhotoDedupProvider>
                  ...
                  return(
                    ...
                    {editingActIdx===savedIdx&&(
                      <div style={{border:isMobile?'none':'1px solid rgba(255,255,255,0.10)',borderRadius:12,background:isMobile?'rgba(255,255,255,0.03)':'rgba(255,255,255,0.04)',padding:isMobile?'12px 10px':16,marginTop:4,marginBottom:14,animation:'slideOpen 0.40s cubic-bezier(0.25,0.46,0.45,0.94)'}}>
                        <div style={{fontSize:12,color:'rgba(201,160,76,0.60)',letterSpacing:2,marginBottom:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontWeight:700}}>BOOKING DETAILS</div>
                        ...
                        <button type="button" onClick={()=>{if(!nAct.name)return;setDet(d=>({...d,activities:d.activities.map(a=>a.actInsightIdx===savedIdx?{...a,name:nAct.name,date:nAct.date,cost:nAct.cost,link:nAct.link,notes:nAct.transit}:a)}));setEditingActIdx(null);}} style={{marginTop:12,padding:'12px 20px',borderRadius:10,border:'none',background:nAct.name?'linear-gradient(135deg,rgba(201,160,76,0.25),rgba(201,160,76,0.15))':'rgba(255,255,255,0.04)',color:nAct.name?'#c9a04c':'rgba(255,255,255,0.20)',fontSize:12,cursor:nAct.name?'pointer':'default',fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",letterSpacing:1.5,fontWeight:700,width:'100%',minHeight:44}}>SAVE</button>
                      </div>
                    )}
                    </div>
                  );
          ...
          {det.activities.filter(a=>!(a.actInsightIdx!=null&&actInsightSavedIdxs.includes(a.actInsightIdx))&&(a.suggestionActivityIdx==null||!suggestion?.activities?.[a.suggestionActivityIdx])).length>0&&<div style={{marginBottom:16}}>
            {det.activities.filter(a=>!(a.actInsightIdx!=null&&actInsightSavedIdxs.includes(a.actInsightIdx))&&(a.suggestionActivityIdx==null||!suggestion?.activities?.[a.suggestionActivityIdx])).map(a=>(
              <div key={a.id} style={{border:planCommitCardBorder,borderRadius:planCommitCardRadius,background:planCommitCardBg,padding:planCommitCardPad,marginBottom:14,display:'flex',flexDirection:'column'}}>
                ...
                <div style={{display:'flex',alignItems:'center',marginBottom:8}}>
                  ...
                  <a href={`https://www.viator.com/search/${encodeURIComponent(segment.name+' '+a.name)}`} target="_blank" rel="noopener noreferrer" title="Open booking link" style={{background:'none',border:'1px solid rgba(94,139,138,0.25)',borderRadius:6,color:'rgba(94,139,138,0.60)',fontSize:11,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",padding:'4px 10px',textDecoration:'none',minHeight:28,display:'flex',alignItems:'center',marginRight:6}}>🔗</a>
                  ...
                  }} style={{background:'none',border:'1px solid rgba(255,255,255,0.15)',borderRadius:6,color:'rgba(255,255,255,0.35)',fontSize:11,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",padding:'4px 10px',cursor:'pointer',minHeight:28}}>✕</button>
                </div>
                <div style={{fontSize:15,fontWeight:700,color:'#FFFFFF',marginBottom:4}}>{a.name}</div>
                {a.provider&&<div style={{fontSize:12,color:'rgba(255,255,255,0.60)',fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",marginBottom:4}}>{a.provider}</div>}
                {a.brief&&<div style={{fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontSize:13,fontStyle:'italic',color:'rgba(255,255,255,0.75)',marginBottom:8,marginTop:4,lineHeight:1.6}}>{a.brief}</div>}
                <div style={{fontSize:13,color:'#C9A04C',fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",display:'flex',gap:8,flexWrap:'wrap',marginBottom:a.tip||a.link?6:0}}>
                  {a.date?<span>{fD(a.date)}</span>:segment.arrival&&<span style={{fontStyle:'italic',color:'rgba(201,160,76,0.60)',fontSize:12}}>within {fD(segment.arrival)}–{fD(segment.departure)}</span>}{a.cost&&<span style={{color:'#c9a04c'}}>Est. ${a.cost}</span>}
                </div>
                {a.tip&&<div style={{fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontSize:12,color:'rgba(255,255,255,0.60)',lineHeight:1.5,marginTop:4}}>💡 {a.tip}</div>}
                {a.link&&<a href={a.link} target="_blank" rel="noopener noreferrer" style={{fontSize:12,color:'#5E8B8A',textDecoration:'none',display:'inline-block',marginTop:6}}>{a.link.replace(/^https?:\/\//,"").slice(0,40)}</a>}
                </div>
            ...
            <div style={{display:'flex',justifyContent:'space-between',padding:'4px 2px'}}>
              <span style={{fontSize:12,color:'rgba(255,255,255,0.60)',fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",letterSpacing:1}}>TOTAL</span>
              <span style={{fontSize:14,fontWeight:600,color:'rgba(201,160,76,0.85)',fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif"}}>${det.activities.reduce((s,a)=>s+(parseFloat(a.cost)||0),0).toLocaleString()}</span>
            </div>
          </div>}
          {det.activities.filter(a=>!(a.actInsightIdx!=null&&actInsightSavedIdxs.includes(a.actInsightIdx))&&(a.suggestionActivityIdx==null||!suggestion?.activities?.[a.suggestionActivityIdx])).length===0&&!actInsight.loading&&actInsight.items.length===0&&!suggestionsLoading&&<div style={{textAlign:'center',padding:'24px 0 16px'}}><div style={{fontFamily:"'Fraunces',serif",fontSize:14,fontStyle:'italic',color:'rgba(201,160,76,0.40)'}}>No activities planned yet — dives, tours, day trips</div></div>}
          ...
          <button type="button" onClick={()=>setActivitiesManualOpen(o=>!o)} style={{width:'100%',textAlign:'left',background:'rgba(255,255,255,0.04)',border:'1px solid rgba(201,160,76,0.35)',borderRadius:10,color:'#c9a04c',fontSize:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontWeight:700,letterSpacing:1,padding:'10px 14px',cursor:'pointer',marginTop:8,marginBottom:4,minHeight:44}}>{activitiesManualOpen?'⌄ Hide manual activity form':'+ Plan your own activity'}</button>
          {activitiesManualOpen&&<div style={{border:isMobile?'none':'1px solid rgba(255,255,255,0.10)',borderRadius:12,background:isMobile?'rgba(255,255,255,0.03)':'rgba(255,255,255,0.04)',padding:isMobile?'12px 10px':16,marginTop:4}}>
            <div style={{fontSize:12,color:'rgba(201,160,76,0.60)',letterSpacing:2,marginBottom:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontWeight:700}}>+ ADD ACTIVITY</div>
            <SDF label="ACTIVITY NAME" value={nAct.name} onChange={v=>setNAct(a=>({...a,name:v}))} placeholder="Dive / temple / hike..." accent="#c9a04c"/>
            <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 1fr',gap:8,marginTop:8,overflow:'hidden'}}>
              <SDF label="DATE" type="date" value={nAct.date} onChange={v=>setNAct(a=>({...a,date:v}))} accent="#c9a04c"/>
              <SDF label="COST ($)" type="number" value={nAct.cost} onChange={v=>setNAct(a=>({...a,cost:v}))} placeholder="0" accent="#c9a04c"/>
            ...
            <div style={{marginTop:8}}><SDF label="BOOKING LINK" value={nAct.link} onChange={v=>setNAct(a=>({...a,link:v}))} placeholder="https://..." accent="#c9a04c"/></div>
            <div style={{marginTop:8}}><SDF label="NOTES" value={nAct.transit} onChange={v=>setNAct(a=>({...a,transit:v}))} placeholder="Tips, what to bring..." accent="#c9a04c" multiline/></div>
            <button type="button" onClick={()=>{if(!nAct.name)return;setDet(d=>({...d,activities:[...d.activities,{...nAct,id:Date.now()}]}));setNAct({name:"",date:"",cost:"",transit:"",link:""});}} style={{marginTop:12,padding:'12px 20px',borderRadius:10,border:'none',background:nAct.name?'linear-gradient(135deg,rgba(201,160,76,0.25),rgba(201,160,76,0.15))':'rgba(255,255,255,0.04)',color:nAct.name?'#c9a04c':'rgba(255,255,255,0.20)',fontSize:12,cursor:nAct.name?'pointer':'default',fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",letterSpacing:1.5,fontWeight:700,width:'100%',minHeight:44}}>ADD TO PLAN</button>
          </div>}
        ...
        {visitedTabs.has("food")&&<div style={{display:tab==="food"?"block":"none"}}>{(()=>{const hasFood=!isRowEmpty(det.food?.dailyBudget);const showFoodSummary=hasFood&&!editingFood&&!foodFocused&&foodInsightSavedIdxs.length===0;const showFoodManualBase=!showFoodSummary&&foodInsightSavedIdxs.length===0&&!(suggestionsLoading&&!suggestion);const showFoodManualForm=(showFoodManualBase&&(foodManualOpen||editingFood))||(foodInsightSavedIdxs.length>0&&editingFood);return(<div style={{padding:0}} onFocus={()=>setFoodFocused(true)} onBlur={(e)=>{if(!e.currentTarget.contains(e.relatedTarget))setFoodFocused(false);}}>
          ...
          {showFoodSummary&&<div style={{border:planCommitCardBorder,borderRadius:planCommitCardRadius,background:planCommitCardBg,padding:planCommitCardPad,marginBottom:14,display:'flex',flexDirection:'column'}}>
            ...
            <div style={{display:'flex',alignItems:'center',marginBottom:8}}>
              ...
              <button type="button" title="Edit details" onClick={()=>setEditingFood(true)} style={{background:'none',border:'1px solid rgba(201,160,76,0.30)',borderRadius:6,color:'rgba(201,160,76,0.70)',fontSize:11,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontWeight:600,letterSpacing:1,padding:'4px 10px',cursor:'pointer',minHeight:28,marginRight:6}}>EDIT</button>
          ...
          {bookDropdown==='food'&&<div style={{position:'relative',zIndex:100,marginBottom:10}}><div style={{background:'rgba(10,7,5,0.95)',border:'1px solid rgba(255,255,255,0.12)',borderRadius:12,padding:8,boxShadow:'0 8px 32px rgba(0,0,0,0.6)'}}>
          ...
          <div style={{display:'flex',gap:10,alignItems:'flex-end',marginBottom:10,flexWrap:isMobile?'wrap':'nowrap'}}>
            <div style={{flex:1,minWidth:0}}><SDF label="DAILY FOOD BUDGET ($)" type="number" value={det.food.dailyBudget} onChange={v=>uF("dailyBudget",v)} placeholder="e.g. 45" accent="#C9A04C"/></div>
            {hasFood&&<button type="button" onClick={()=>{if(typeof document!=="undefined")document.activeElement?.blur();setFoodFocused(false);setEditingFood(false);}} style={{padding:'8px 14px',borderRadius:6,border:'1px solid rgba(201,160,76,0.30)',background:'none',color:'rgba(201,160,76,0.70)',fontSize:12,cursor:'pointer',fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",letterSpacing:1,fontWeight:600,whiteSpace:'nowrap',height:34,flexShrink:0}}>SAVE</button>}
            <button type="button" onClick={aiFood} disabled={aiLoad} style={{padding:'8px 14px',borderRadius:6,border:'1px solid rgba(201,160,76,0.3)',background:'rgba(201,160,76,0.05)',color:'rgba(201,160,76,0.8)',fontSize:12,cursor:aiLoad?'wait':'pointer',fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",letterSpacing:1,fontWeight:600,whiteSpace:'nowrap',height:34,flexShrink:0}}>{aiLoad?"✦...":"✦ CO-ARCH EST"}</button>
          </div>
          {hasFood&&<div style={{display:'flex',justifyContent:'space-between',padding:'10px 14px',background:'rgba(201,160,76,0.05)',border:'1px solid rgba(201,160,76,0.16)',borderRadius:8,marginBottom:10}}>
            <span style={{fontSize:13,color:'rgba(255,255,255,0.60)',fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif"}}>{segment.nights} nights × ${det.food.dailyBudget}/day</span>
            <span style={{fontSize:14,fontWeight:600,color:'rgba(201,160,76,0.85)',fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif"}}>${(parseFloat(det.food.dailyBudget)*segment.nights).toLocaleString()}</span>
          </div>}
          <SDF label="FOOD NOTES" value={det.food.notes} onChange={v=>uF("notes",v)} placeholder="Must-try dishes, market days, dietary notes..." accent="#C9A04C" multiline/>
          {hasFood&&editingFood&&<div style={{textAlign:'left',marginTop:10}}>
            <button type="button" onClick={()=>{setEditingFood(false);}} style={{background:'none',border:'1px solid rgba(201,160,76,0.30)',borderRadius:6,color:'rgba(201,160,76,0.70)',fontSize:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontWeight:600,letterSpacing:1,padding:'6px 14px',cursor:'pointer',minHeight:30,lineHeight:1.3}}>DONE</button>
          </div>}
          </>}
          </>}
          <button type="button" onClick={()=>setOwnRestaurantOpen(o=>!o)} style={{width:'100%',border:'1px dashed rgba(212,175,55,0.35)',borderRadius:12,padding:'14px 16px',textAlign:'center',cursor:'pointer',color:'#C9A04C',fontSize:14,fontWeight:600,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",letterSpacing:0.5,marginTop:12,background:'rgba(212,175,55,0.04)',transition:'all 0.30s cubic-bezier(0.25,0.46,0.45,0.94)'}}>+ Add your own restaurant</button>
          {ownRestaurantOpen&&<div style={{border:isMobile?'none':'1px solid rgba(255,255,255,0.10)',borderRadius:12,background:isMobile?'rgba(255,255,255,0.03)':'rgba(255,255,255,0.04)',padding:isMobile?'12px 10px':16,marginTop:10,marginBottom:4,animation:'slideOpen 0.40s cubic-bezier(0.25,0.46,0.45,0.94)'}}>
            <div style={{fontSize:12,color:'rgba(201,160,76,0.60)',letterSpacing:2,marginBottom:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontWeight:700}}>YOUR RESTAURANT</div>
            <SDF label="RESTAURANT NAME" value={ownRestaurant.name} onChange={v=>setOwnRestaurant(r=>({...r,name:v}))} placeholder="Name or neighborhood" accent="#C9A04C"/>
            <div style={{marginTop:8}}><SDF label="CUISINE TYPE (OPTIONAL)" value={ownRestaurant.cuisine} onChange={v=>setOwnRestaurant(r=>({...r,cuisine:v}))} placeholder="e.g. Vietnamese, omakase" accent="#C9A04C"/></div>
            <div style={{marginTop:8}}><SDF label="PRICE RANGE" value={ownRestaurant.priceTier} onChange={v=>setOwnRestaurant(r=>({...r,priceTier:v}))} placeholder="$ · $$ · $$$" accent="#C9A04C"/></div>
            <div style={{marginTop:8}}><SDF label="BOOKING LINK" value={ownRestaurant.link} onChange={v=>setOwnRestaurant(r=>({...r,link:v}))} placeholder="https://..." accent="#C9A04C"/></div>
            <div style={{marginTop:8}}><SDF label="NOTES" value={ownRestaurant.notes} onChange={v=>setOwnRestaurant(r=>({...r,notes:v}))} placeholder="Reservation time, dishes to try..." accent="#C9A04C" multiline/></div>
            <button type="button" onClick={()=>{if(!ownRestaurant.name.trim())return;const t=(ownRestaurant.priceTier||"").trim();const est=t.includes("$$$")?"85":t.includes("$$")?"48":t.includes("$")?"28":"";const block=[`🍽 ${ownRestaurant.name.trim()}`,[ownRestaurant.cuisine,t].filter(Boolean).join(" · "),ownRestaurant.link?`Link: ${ownRestaurant.link}`:"",ownRestaurant.notes||""].filter(Boolean).join("\n");setDet(d=>{const prev=parseFloat(d.food.dailyBudget)||0;const nextBud=est?String(Math.max(prev,parseInt(est,10)||0)):d.food.dailyBudget;return{...d,food:{...d.food,dailyBudget:nextBud,notes:d.food.notes?`${d.food.notes}\n\n${block}`:block}};});setOwnRestaurant({name:"",cuisine:"",priceTier:"",link:"",notes:""});setOwnRestaurantOpen(false);}} style={{marginTop:12,padding:'12px 20px',borderRadius:10,border:'none',background:ownRestaurant.name.trim()?'linear-gradient(135deg,rgba(201,160,76,0.25),rgba(201,160,76,0.15))':'rgba(255,255,255,0.04)',color:ownRestaurant.name.trim()?'#c9a04c':'rgba(255,255,255,0.20)',fontSize:12,cursor:ownRestaurant.name.trim()?'pointer':'default',fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",letterSpacing:1.5,fontWeight:700,width:'100%',minHeight:44}}>SAVE</button>
          </div>}
        </div>);})()}</div>}
        {visitedTabs.has("itinerary")&&<div style={{display:tab==="itinerary"?"block":"none"}}><ItineraryTab segment={segment} det={det} fmt={fmt}/></div>}
        {/* BUDGET */}
        {visitedTabs.has("budget")&&<div style={{display:tab==="budget"?"block":"none"}}>{(()=>{const tCost=parseFloat(det.transport?.cost)||0;const sCost=parseFloat(det.stay?.cost)||0;const aCost=det.activities.reduce((s,a)=>s+(parseFloat(a.cost)||0),0);const fCost=(parseFloat(det.food?.dailyBudget)||0)*segment.nights;const mCost=det.misc.reduce((s,m)=>s+(parseFloat(m.cost)||0),0);const total=tCost+sCost+aCost+fCost+mCost;const budget=segment.budget||0;const pct=budget>0?Math.round((total/budget)*100):0;const barColor=pct>=100?'#FF6B6B':pct>=80?'#c9a04c':'#5E8B8A';
          ...
          <div style={{padding:0}}>
            ...
            <div style={{fontSize:13,fontFamily:'Fraunces, serif',fontWeight:600,color:'rgba(201,160,76,0.78)',letterSpacing:2,marginBottom:12,textAlign:'left',lineHeight:1.4}}>PHASE BUDGET</div>
            ...
            <div style={{fontSize:13,color:'rgba(255,255,255,0.55)',fontFamily:'Instrument Sans, system-ui, sans-serif',marginBottom:16,textAlign:'left'}}>{segment.nights} Nights · Budget: {fmt(budget)}</div>
            {budgetRows.map(r=>{
              const snapStyle={fontSize:isMobile?13:14,fontStyle:'normal',fontWeight:500,color:'rgba(255,245,220,0.88)',marginTop:7,lineHeight:1.55,wordBreak:'break-word',letterSpacing:'0.02em',textAlign:'left',fontFamily:'Instrument Sans, system-ui, sans-serif'};
              return(
              <div key={r.label} style={{display:'flex',alignItems:'flex-start',gap:isMobile?8:12,padding:isMobile?'14px 0':'15px 0',borderBottom:'1px solid rgba(255,255,255,0.06)'}}>
                ...
                <div style={{flex:1,minWidth:0,textAlign:'left'}}>
                  <div style={{fontSize:isMobile?15:17,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",color:'rgba(255,255,255,0.90)',letterSpacing:isMobile?0.8:1.2,fontWeight:700,textAlign:'left'}}>{r.label}</div>
                  {r.snapLines&&r.snapLines.length>0&&<div style={{marginTop:7}}>{r.snapLines.map((line,i)=><div key={i} style={{...snapStyle,marginTop:i?10:0,display:'flex',gap:10,alignItems:'baseline'}}><span style={{flexShrink:0,color:'rgba(201,160,76,0.55)',fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontSize:12,lineHeight:1,fontStyle:'normal',fontWeight:700}}>●</span><span style={{flex:1,minWidth:0}}>{line}</span></div>)}</div>}
                ...
                <div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',flexShrink:0,gap:5}}>
                  <span style={{fontSize:isMobile?16:18,fontWeight:700,color:'#FFFFFF',fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",whiteSpace:'nowrap'}}>{r.cost>0?fmt(r.cost):'—'}</span>
                  <span style={{fontSize:isMobile?13:14,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontWeight:600,color:r.has?'#69F0AE':'rgba(255,255,255,0.60)',letterSpacing:0.5,whiteSpace:'nowrap',lineHeight:1.35}}>{r.has?'✓ Added':'—'}</span>
                </div>
              </div>
              );
            })}
            <div style={{display:'flex',justifyContent:'space-between',padding:'14px 0 6px',borderTop:'1px solid rgba(255,255,255,0.12)',marginTop:4}}>
              <span style={{fontSize:13,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",color:'rgba(255,255,255,0.65)',letterSpacing:1}}>TOTAL PLANNED</span>
              <span style={{fontSize:15,fontWeight:700,color:'#FFFFFF',fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif"}}>{fmt(total)}</span>
            </div>
            <div style={{display:'flex',justifyContent:'space-between',padding:'6px 0'}}>
              <span style={{fontSize:13,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",color:'rgba(255,255,255,0.65)',letterSpacing:1}}>REMAINING</span>
              <span style={{fontSize:15,fontWeight:700,color:budget-total>=0?'#69F0AE':'#FF6B6B',fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif"}}>{fmt(budget-total)}</span>
            </div>
            ...
            <div style={{textAlign:'center',marginTop:6,fontSize:13,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",color:barColor,lineHeight:1.4}}>{pct}% allocated</div>
        ...
        {visitedTabs.has("docs")&&<div style={{display:tab==="docs"?"block":"none",padding:0}}>
          ...
          <div style={{fontSize:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",color:'rgba(201,160,76,0.65)',letterSpacing:2,marginBottom:12}}>DOCS & VISA</div>
          <div style={{fontSize:15,fontWeight:700,color:'#FFFFFF',marginBottom:4}}>{segment.name}, {segment.country}</div>
          <div style={{fontSize:13,color:'rgba(255,255,255,0.55)',fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",marginBottom:16}}>{segment.nights} Nights</div>
          {!docsData&&!docsLoading&&<div style={{textAlign:'left',padding:'16px 0 8px'}}><button type="button" onClick={loadDocs} style={{padding:'12px 24px',borderRadius:10,border:'1px solid rgba(201,160,76,0.40)',background:'rgba(201,160,76,0.08)',color:'#C9A04C',fontSize:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",fontWeight:700,letterSpacing:1,cursor:'pointer',minHeight:44}}>✦ GENERATE TRAVEL DOCS BRIEF</button></div>}
          {docsLoading&&<div style={{textAlign:'left',padding:'16px 0 8px',display:'flex',alignItems:'center',gap:10}}><div style={{width:8,height:8,borderRadius:'50%',background:'rgba(201,160,76,0.6)',animation:'pulse 1.5s ease-in-out infinite',flexShrink:0}}/><span style={{fontSize:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",color:'rgba(255,255,255,0.60)',letterSpacing:1}}>GENERATING DOCS BRIEF...</span></div>}
          {docsData&&<div style={{display:'flex',flexDirection:'column',gap:14}}>
            {docsData.visa&&<div><div style={{fontSize:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",color:'#C9A04C',letterSpacing:2,marginBottom:6}}>🛂 VISA REQUIREMENTS</div><div style={{fontFamily:"'Fraunces',serif",fontSize:14,color:'rgba(255,255,255,0.85)',lineHeight:1.7}}>{docsData.visa.details}{docsData.visa.cost?` · Cost: ${docsData.visa.cost}`:''}</div></div>}
            {docsData.health&&<div><div style={{fontSize:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",color:'#C9A04C',letterSpacing:2,marginBottom:6}}>💉 HEALTH</div>{docsData.health.required?.length>0&&<div style={{padding:'6px 10px',background:'rgba(201,160,76,0.06)',border:'1px solid rgba(201,160,76,0.20)',borderRadius:6,marginBottom:4,fontSize:13,color:'#c9a04c'}}>⚠️ Required: {docsData.health.required.join(', ')}</div>}{docsData.health.recommended?.length>0&&<div style={{fontSize:13,color:'rgba(255,255,255,0.75)',lineHeight:1.5}}>Recommended: {docsData.health.recommended.join(', ')}</div>}{docsData.health.notes&&<div style={{fontSize:12,color:'rgba(255,255,255,0.55)',marginTop:4}}>{docsData.health.notes}</div>}</div>}
            {docsData.money&&<div><div style={{fontSize:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",color:'#C9A04C',letterSpacing:2,marginBottom:6}}>💵 MONEY & CURRENCY</div>{docsData.money.warning&&<div style={{padding:'6px 10px',background:'rgba(201,160,76,0.06)',border:'1px solid rgba(201,160,76,0.20)',borderRadius:6,marginBottom:4,fontSize:13,color:'#c9a04c'}}>⚠️ {docsData.money.warning}</div>}<div style={{fontSize:13,color:'rgba(255,255,255,0.80)',lineHeight:1.5}}>{docsData.money.currency}{docsData.money.tips?` · ${docsData.money.tips}`:''}</div></div>}
            {docsData.connectivity&&<div><div style={{fontSize:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",color:'#C9A04C',letterSpacing:2,marginBottom:6}}>📶 CONNECTIVITY</div><div style={{fontSize:13,color:'rgba(255,255,255,0.75)',lineHeight:1.5}}>{docsData.connectivity.tips}</div></div>}
            {docsData.safety&&<div><div style={{fontSize:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",color:'#C9A04C',letterSpacing:2,marginBottom:6}}>🛡️ SAFETY</div><div style={{fontSize:13,color:'rgba(255,255,255,0.75)',lineHeight:1.5}}>Level: {docsData.safety.level}{docsData.safety.notes?` · ${docsData.safety.notes}`:''}</div></div>}
            {docsData.customs&&<div><div style={{fontSize:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",color:'#C9A04C',letterSpacing:2,marginBottom:6}}>🤝 LOCAL CUSTOMS</div><div style={{fontSize:13,color:'rgba(255,255,255,0.75)',lineHeight:1.5}}>{docsData.customs.tips}</div></div>}
            {docsData.emergency&&<div><div style={{fontSize:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",color:'#C9A04C',letterSpacing:2,marginBottom:6}}>🆘 EMERGENCY CONTACTS</div><div style={{display:'flex',flexDirection:'column',gap:4}}>{docsData.emergency.police&&<div style={{fontSize:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",color:'#5E8B8A'}}>Police: {docsData.emergency.police}</div>}{docsData.emergency.ambulance&&<div style={{fontSize:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",color:'#5E8B8A'}}>Ambulance: {docsData.emergency.ambulance}</div>}{docsData.emergency.embassy&&<div style={{fontSize:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",color:'#5E8B8A'}}>Embassy: {docsData.emergency.embassy}</div>}</div></div>}
          </div>}
          <div style={{marginTop:16}}><SDF label="PERSONAL NOTES" value={docsNote} onChange={v=>{setDocsNote(v);setDet(d=>({...d,intel:{...d.intel,notes:v}}));}} placeholder="Visa application status, insurance details, personal contacts..." accent="#C9A04C" multiline/></div>
        </div>}
        {visitedTabs.has("calendar")&&<div style={{display:tab==="calendar"?"block":"none"}}>{(()=>{const items=[];if(det.transport.mode||det.transport.from){const mode=(det.transport.mode||"Transport").trim();const fromTo=`${(det.transport.from||"").trim()}→${(det.transport.to||"").trim()}`;const desc=truncateCalendarLine(`${mode} · ${fromTo}`);items.push({type:"transport",icon:"✈️",label:mode||"Transport",desc,color:"#5E8B8A",date:segment.arrival});}if(det.stay.name){const noteHead=sanitizeAiDisplayText((det.stay.notes||"").split(/[\n.]/)[0]||"").slice(0,28).trim();const desc=truncateCalendarLine(noteHead?`${noteHead} · ${segment.name||""}`.trim():`Stay · ${segment.name||"destination"}`);items.push({type:"stay",icon:"🏨",label:det.stay.name,desc,color:"#69F0AE",date:det.stay.checkin||segment.arrival});}det.activities.forEach(a=>{const cat=(a.provider||"Activity").trim();const brief=(a.brief||"").trim()||(String(a.notes||"").split("\n")[0]||"").trim();const desc=truncateCalendarLine(brief?`${cat} · ${brief}`:`${cat} · self-guided`);items.push({type:"activity",icon:"🎯",label:a.name,desc,color:"#c9a04c",date:a.date});});if(det.food.dailyBudget){const foodHint=sanitizeAiDisplayText((det.food.notes||"").split("\n")[0]||"").trim();const desc=truncateCalendarLine(foodHint||`Daily dining · $${det.food.dailyBudget}/day`);items.push({type:"food",icon:"🍜",label:`Food · $${det.food.dailyBudget}/day`,desc,color:"#C9A04C"});}const hasItems=items.length>0;return(
          <div style={{padding:0}}>
            {!hasItems&&<div style={{textAlign:"center",padding:"40px 20px",minHeight:"auto"}}>
              ...
              <div style={{color:"rgba(201,160,76,0.6)",fontSize:12,letterSpacing:2,marginTop:8,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif"}}>ADD TRANSPORT · STAY · ACTIVITIES · FOOD TO SEE YOUR DAYS</div>
            </div>}
            {hasItems&&<div>
              <div style={{fontSize:12,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",color:"rgba(201,160,76,0.65)",letterSpacing:2,marginBottom:12}}>PLANNED ITEMS</div>
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {items.map((it,i)=><div key={`${it.type}-${i}-${String(it.label||'').slice(0,24)}`} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",background:`${it.color}08`,border:`1px solid ${it.color}30`,borderRadius:10}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:15,fontWeight:600,color:"#FFF",fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif"}}>{it.label}</div>
                    {it.desc&&<div style={{fontSize:11,color:"rgba(255,255,255,0.45)",fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",marginTop:3,lineHeight:1.35}}>{it.desc}</div>}
                  </div>
                  {it.date&&<div style={{fontSize:12,color:it.color,fontFamily:"'Instrument Sans',system-ui,-apple-system,sans-serif",flexShrink:0,whiteSpace:"nowrap"}}>{fD(it.date)}</div>}
                </div>)}
              </div>
            </div>}
          </div>
