        ...
        {(!isMobile||mobileTab==="chat")&&(
          <div style={{flex:1,display:"flex",flexDirection:"column",borderRight:isMobile?"none":"1px solid #111D2A",...(isMobile?{flex:1,borderTop:"1px solid #111D2A"}:{})}}>
              ...
              {chat.map((msg,i)=>(
                <div key={i} style={{display:"flex",gap:8,alignItems:"flex-start",flexDirection:msg.role==="user"?"row-reverse":"row",animation:"msgIn 0.25s ease"}}>
                  ...
                  {msg.isWelcome
                    ...
                    :<div style={{background:msg.role==="ai"?"rgba(169,70,29,0.18)":"rgba(255,255,255,0.05)",border:msg.role==="ai"?"1px solid rgba(255,217,61,0.35)":`1px solid rgba(255,255,255,0.08)`,borderRadius:12,padding:msg.role==="ai"?(isMobile?"14px 12px":"18px 20px"):"10px 14px",fontSize:msg.role==="ai"?(isMobile?14:17):13,fontFamily:msg.role==="ai"?"'Playfair Display',serif":"'Inter',system-ui,-apple-system,sans-serif",fontStyle:msg.role==="ai"?"italic":"normal",fontWeight:msg.role==="ai"?400:400,color:msg.role==="ai"?"#FAF6EF":"#FFF",lineHeight:msg.role==="ai"?(isMobile?1.58:1.68):1.5,maxWidth:isMobile?"100%":"92%",boxShadow:msg.role==="ai"?"inset 0 0 22px rgba(0,0,0,0.2),0 0 20px rgba(169,70,29,0.12)":"none",textShadow:msg.role==="ai"?"0 1px 2px rgba(0,0,0,0.35)":"none"}}>{(msg.text||"").replace(/\*\*(.*?)\*\*/g,'$1').replace(/\*(.*?)\*/g,'$1')}</div>}
                </div>
              ))}
            ...
            <div style={{padding:isMobile?"8px 12px":"8px 10px",borderTop:"1px solid #111D2A",display:"flex",gap:7,flexShrink:0}}>
              <input style={{flex:1,background:"rgba(169,70,29,0.18)",border:"1px solid rgba(255,217,61,0.35)",borderRadius:8,color:"#FFF",fontSize:isMobile?13:15,padding:"8px 10px",fontFamily:"'Inter',system-ui,-apple-system,sans-serif",outline:"none",minHeight:44,transition:"border-color 0.30s cubic-bezier(0.25,0.46,0.45,0.94),box-shadow 0.30s cubic-bezier(0.25,0.46,0.45,0.94)"}} value={input} onChange={e=>setInput(e.target.value)} onClick={e=>e.stopPropagation()} onKeyDown={e=>{if(e.key==="Enter"){e.preventDefault();sendMsg();}}} placeholder="Now let's dial it in — what are you thinking?" onFocus={e=>{e.target.style.borderColor="rgba(255,217,61,0.65)";e.target.style.boxShadow="0 0 0 2px rgba(255,217,61,0.15)";}} onBlur={e=>{e.target.style.borderColor="rgba(255,217,61,0.35)";e.target.style.boxShadow="none";}}/>
